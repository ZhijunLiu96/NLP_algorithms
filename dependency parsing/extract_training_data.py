...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in range(1, 4):
            if i < len(stack):
                ...
                input0.append(4)
        for i in range(1, 4):
            if i < len(buffer) and buffer[0] != 0:
                word = words[buffer[-i]]
                tag = pos[buffer[-i]]
                if word in self.word_vocab.keys():
                    index = self.word_vocab[word]
                    ...
                    input0.append(2)
            elif i == len(buffer) and buffer[0] == 0:
                input0.append(3)
            else:
                input0.append(4)
    ...
    def get_output_representation(self, output_pair):  
        ...
        value = labels[output_pair]
        #label = np.arange(91)
        #pair = keras.utils.to_categorical(label, num_classes=91, dtype='float32')
        #vector = pair[value]
        return keras.utils.to_categorical(value, num_classes=91)
